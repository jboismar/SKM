...
if( !isset($_GET["hostgroup"])) { $hostgroup="All Hosts"; }
...
<?php start_main_frame("<a href=\"hosts-view.php\"> SKM </a> > $hostgroup"); ?>
<?php start_left_pane(); ?>
<?php display_menu(); ?>
<?php end_left_pane(); ?>
...
<!-- <form name="hostnameSearch" action="hosts-view.php" method="post">
  ...
  <table class=detail>
    ...
    <?php
    if ($hostgroup == "All Hosts") {
    	$SQLQUERY="SELECT * FROM `hosts` WHERE `name` like '$hostname' ORDER BY `name` ASC";
    } else {
    	$SQLQUERY="SELECT * FROM `hosts` where `Environment` = '$hostgroup' AND `name` like '$hostname' ORDER BY `name` ASC";
    }
